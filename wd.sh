...
#   wd.sh answered <turn end_ts> --acknowledged <target delivery uuid> "<what was acknowledged>"  record a real standalone reply to this turn
...
case "$cmd" in
  ...
  promise) exec $PY "$D/wd_check.py" "${CHECK_ARGS[@]}" promise "$@" ;;
